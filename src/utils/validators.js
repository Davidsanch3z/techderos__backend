...
function validateColombianPhone(phone) {
  ...
  if (!phoneRegex.test(cleanPhone)) {
    return {
      isValid: false,
      message:
        "El teléfono debe tener 10 dígitos y comenzar con 3 (formato Colombia)",
    };
  }
      ...
      errors.length === 0
        ? {
            ...
            telefono: userData.telefono
              ? validateColombianPhone(userData.telefono).cleanPhone
              : null,
...
function isBase64(str) {
  ...
  try {
    ...
    // Regex para caracteres válidos base64
    const notBase64 = /[^A-Z0-9+\/=]/i;
    if (notBase64.test(str)) return false;

    // Intentar decodificar y volver a codificar
    const decoded = Buffer.from(str, "base64").toString("base64");
    return decoded === str.replace(/=+$/, (m) => "=".repeat(m.length)); // tolera padding
  } catch {
    return false;
  }
}
