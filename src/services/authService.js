...
const { ValidationError, AuthenticationError, NotFoundError } = require('../utils/helpers');
...
class AuthService {
  ...
  async register(userData) {
    try {
      const { nombre, email, password, telefono, tipo_negocio } = userData;
      ...
      const user = new User({
        ...
        email,
        roleId: 'user', // Por defecto rol de usuario/tendero
  ...
  async login(email, password, ipAddress = null) {
    try {
      ...
      if (user.isAccountLocked()) {
        ...
        throw new AuthenticationError(`Cuenta bloqueada. Intenta nuevamente en ${minutesLeft} minutos.`);
      }
      ...
      if (!user.isActive) {
        throw new AuthenticationError('Tu cuenta no está activa. Contacta al administrador.');
      }
  ...
  async generateTokens(user) {
    ...
    return {
      accessToken,
      refreshToken,
      expiresIn: this.getTokenExpiryTime(process.env.JWT_ACCESS_EXPIRY || '15m')
    };
  }
